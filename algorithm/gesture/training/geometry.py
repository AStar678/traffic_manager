...
# 63 normalized landmarks + 60 bone vectors + 15 finger direction values
# + 15 joint-angle cosines + 10 fingertip distances + 3 image palm normal
# + 3 world palm normal + 6 global/detection values.
...
def _palm_normal(points: np.ndarray) -> np.ndarray:
    index_vector = points[5] - points[0]
    pinky_vector = points[17] - points[0]
    return _safe_unit(np.cross(index_vector, pinky_vector))


def _joint_cosine(a: np.ndarray, b: np.ndarray, c: np.ndarray) -> float:
    first = _safe_unit(a - b)
    second = _safe_unit(c - b)
...
def extract_geometry_features(
    ...
    *,
    ...
    """Return a scale-normalized 175-D feature vector for one video frame.

    ``landmarks`` and ``world_landmarks`` must use MediaPipe's standard hand
    order and have shape ``[21, 3]``. ``handedness`` is -1 for left, +1 for
    right and 0 when unknown.
    """
    ...
    bone_vectors = np.concatenate(
        [(points[end] - points[start]) / scale for start, end in HAND_EDGES]
    )
    finger_vectors = np.concatenate(
        [_safe_unit(points[tip] - wrist) for tip in FINGERTIPS]
    )
