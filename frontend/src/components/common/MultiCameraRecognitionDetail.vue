<template>
  <div class="multi-camera-detail">
      ...
      >
        ...
        <div class="camera-result-viewport">
          <video
            ...
            playsinline
          ></video>
          <img v-else-if="cameraPreviewUrls[camera.slotId]" :src="cameraPreviewUrls[camera.slotId]" :alt="`摄像头 ${camera.slotId}`">
          ...
          <svg
            v-if="taskType === 'license_plate' && (cameraWebRtcStreams[camera.slotId] || cameraPreviewUrls[camera.slotId]) && overlayDetections(camera).length"
          ...
          <svg
            v-if="taskType === 'police_gesture' && (cameraWebRtcStreams[camera.slotId] || cameraPreviewUrls[camera.slotId]) && policeSkeletons(camera).length"
          ...
          <div class="camera-frame-label">
            ...
            <small>{{ cameraWebRtcStreams[camera.slotId] ? 'WebRTC 低延迟' : 'JPEG 降级' }}</small>
          </div>
        </div>
    ...
    <footer class="detail-summary">
      ...
      <div><span>传输方式</span><strong>WebRTC / UDP</strong></div>
    </footer>
  </div>
</template>
...
const { cameraSlots, cameraPreviewUrls, cameraWebRtcStreams } = useCameraSource()
const policeBones = [
  ['nose', 'neck'],
...
.police-skeleton-overlay { position: absolute; inset: 0; z-index: 2; width: 100%; height: 100%; pointer-events: none; }
